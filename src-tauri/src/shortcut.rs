...
async fn fetch_models_manual(
    ...
    headers.insert(
        "HTTP-Referer",
        reqwest::header::HeaderValue::from_static("https://github.com/Vasista03/Nugget"),
    );
    headers.insert(
        "X-Title",
        reqwest::header::HeaderValue::from_static("Nugget"),
    );
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    app.global_shortcut().unregister(shortcut).map_err(|e| {
        ...
        // Unregister failures are often non-fatal (e.g. when the OS has
        // already released the hotkey), so log at warn level instead of error
        // to avoid noisy error logs while still surfacing unexpected issues.
        warn!("_unregister_shortcut error: {}", error_msg);
        error_msg
    })?;
